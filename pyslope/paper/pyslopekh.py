...
class Slope:
    ...
    def __init__(self, height: float = 1, angle: float = 30, length: float = None, Kh: float = 0.1):
    ...
    def set_external_boundary(
        self, height: float = 1, angle: float = 30, length: float = None
    ):
    ...
    def analyse_slope(self, max_fos=None):
        ...
        if self._slices * self._iterations < 25000:
            for i, search in enumerate(self._search):
    ...
    def _analyse_circular_failure_ordinary(
        ...
        for _ in range(0, SLICES):
            ...
            gc = 0.0 # center of gravity
            ugc = 0.0
            lgc = 0.0
            total_thick = round(s_yt - s_yb)
            #print(s_yt,s_yb)
            for i in self._materials[0:total_thick]:
                ugc += i.unit_weight * i.depth_to_bottom
                lgc += i.unit_weight
                gc = ugc / lgc

            Kh = self.Kh
            # calculate resisting, modified ref: https://www.jstage.jst.go.jp/article/jscejc/63/3/63_3_677/_pdf
            resisting += cohesion * inclined_length + max(
                0, (W * cos(alpha) - U - Kh * W * sin(alpha))
            ) * tan(radians(friction_angle))
            
            # calculate pushing
            # pushing += W * sin(alpha) + W *Kh * (c_y - s_yt - gc) / radius
            pushing += W * sin(alpha) + W *Kh * (c_y - s_yt - gc) / radius
    ...
    def _analyse_circular_failure_bishop(
        ...
        for _ in range(self._max_iterations):
            ...
            for _ in range(0, SLICES):
                ...
                # calculate ma, modified ref: https://scientiairanica.sharif.edu/article_1899_5597f0b014b5b80bfc2f11d556f93c5d.pdf
                ...
                # calculate pushing
                pushing_noKh += W*sin(alpha)
                pushing_Kh   += W*(c_y - s_yt - gc)/radius

                # initialise slice x coordinate for next loop
                s_x = s_x + b
                #print(resisting / pushing, (s_yt - s_yb)/2, gc)

            if pushing_noKh <= 0:
                return None
            if resisting < 0 or pushing_noKh < 0:
                return None

            # Now we have:
            # FS=1 => resisting = pushing
            # pushing = pushing_noKh + Kh * pushing_Kh
            # so: resisting = pushing_noKh + Kh * pushing_Kh
            # => Kh = (resisting - pushing_noKh)/pushing_Kh

            if pushing_Kh == 0:
                # If pushing_Kh is zero, can't solve for Kh
                return None

            pre_Kh = (resisting - pushing_noKh)/pushing_Kh
    ...
    def _calculate_strip_weight(self, b: float, s_yt: float, s_yb: float):
        ...
        top = s_yt
        ## Modified to constant Material thickness
        ...
        for m in self._materials:
            ...
            # if m.RL - self._external_height - s_yt >= s_yt:
            #     continue     
            ...
            if m.RL - m.thick  > s_yb:
                W += b * m.unit_weight * m.thick
                top = m.RL - m.thick
...
if __name__ == "__main__":
    s = Slope(height=10, angle=10.2)

    m1 = Material(14, 30, 5, 1) 
    m2 = Material(15, 31, 7, 2) 
    m3 = Material(15, 33, 10, 3)
    m4 = Material(15, 33, 13, 4)
    m5 = Material(15, 34, 15, 5)
    m6 = Material(15, 35, 15, 6)
    m7 = Material(16, 36, 18, 7)
    m8 = Material(16, 37, 18, 8)
    m9 = Material(16, 38, 23, 10)
    m10 = Material(16, 39, 30, 12)

    s.set_materials(m1, m2, m3,m4,m5,m6,m7,m8,m9,m10)

    s.update_analysis_options(slices=20, iterations=500)
    s.set_analysis_limits(s.get_top_coordinates()[0]-0.1, s.get_bottom_coordinates()[0]+0.1)
    s.set_water_table(0)
    s.analyse_slope()
